...
public class BlockDeferredRegister extends DeferredRegisterWrapper<Block> {
   ...
   */
  public <B extends Block> DeferredHolder<Object, B> registerNoItem(String name, Supplier<? extends B> block) {
   ...
   * @return  Block registry object
   */
  public DeferredHolder<Object, Block> registerNoItem(String name, BlockBehaviour.Properties props) {
  ...
  public <B extends Block> ItemObject<B> register(String name, Supplier<? extends B> block, final Function<? super B, ? extends BlockItem> item) {
    DeferredHolder<Object, B> blockObj = registerNoItem(name, block);
  ...
  public WoodBlockObject registerWood(String name, Function<WoodVariant,BlockBehaviour.Properties> behaviorCreator, boolean flammable) {
    ...
    // signs
    DeferredHolder<Object, StandingSignBlock> standingSign = registerNoItem(name + "_sign", () -> new MantleStandingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F), woodType));
    DeferredHolder<Object, WallSignBlock> wallSign = registerNoItem(name + "_wall_sign", () -> new MantleWallSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F).lootFrom(standingSign), woodType));
    DeferredHolder<Object, MantleCeilingHangingSignBlock> hangingSign = registerNoItem(name + "_hanging_sign", () -> new MantleCeilingHangingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F), woodType));
    DeferredHolder<Object, MantleWallHangingSignBlock> wallHangingSign = registerNoItem(name + "_wall_hanging_sign", () -> new MantleWallHangingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F).lootFrom(hangingSign), woodType));
   ...
   */
  public DeferredHolder<Object, FlowerPotBlock> registerPotted(String name, Supplier<? extends Block> block) {
    DeferredHolder<Object, FlowerPotBlock> potted = registerNoItem("potted_" + name, () -> new FlowerPotBlock(() -> (FlowerPotBlock)Blocks.FLOWER_POT, block, POTTED_PROPS));
    ((FlowerPotBlock)Blocks.FLOWER_POT).addPlant(resource(name), potted);
    return potted;
  }

  /** Registers a potted form of the given block using the vanilla pot */
  public DeferredHolder<Object, FlowerPotBlock> registerPotted(DeferredHolder<Object, ? extends Block> block) {
    return registerPotted(block.getId().getPath(), block);
  }

  /** Registers a potted form of the given block using the vanilla pot */
  public DeferredHolder<Object, FlowerPotBlock> registerPotted(ItemObject<? extends Block> block) {
    return registerPotted(block.getId().getPath(), block);
  }
  ...
  public MetalItemObject registerMetal(String name, String tagName, Supplier<Block> blockSupplier, Function<Block,? extends BlockItem> blockItem, Item.Properties itemProps) {
    ...
    DeferredHolder<Object, Item> ingot = itemRegister.register(name + "_ingot", itemSupplier);
    DeferredHolder<Object, Item> nugget = itemRegister.register(name + "_nugget", itemSupplier);
